...

{
  ...
  val stars = IndexedSeq(
    ...
    // 冬の星座
    ("03h08m", "ペルセウス座アルゴル(2等星)が南中", Nil),
    ("03h24m", "ペルセウス座α(2等星)が南中", Nil),
    ...
    ("05h26m", "おうし座βが南中。おうし座の角であり、ぎょしゃ座の五角形の1つでもあります", Nil),
    ...
    ("06h38m", "ふたご座γが南中。ふたご座の東側の子の足元の星です", Nil),
    ...
    ("12h23m", "かみのけ座の散開星団が南中。りょうけん座コル・カロリ(3等星)としし座デネブの中間にあります。かみのけ座は、この散開星団を中心とした目立たない星座で、銀河北極点に位置します。かみのけ座銀河団もあります", Nil),
    ("12h56m", "りょうけん座コル・カロリ(3等星)が南中。春の大三角形であるうしかい座アークトゥルス、おとめ座スピカ、しし座デネブにコル・カロリを加えると、春のダイヤモンドです", Nil),
    ...
    ("17h00m", "ヘラクレス座が南中。ヘラクレス座は、大きな星座ですが、明るい星はありません。ヘラクレス座の西はかんむり座、南はへびつかい座、東はこと座、北はりゅう座です", Nil),
    ...
    // 夏の大三角形
    ("18h37m", "こと座ベガが南中。天頂付近にいます。夏の大三角形の1つです。ベガの東はわし座アルタイルです", Nil),
    ("19h51m", "わし座アルタイルが南中。夏の大三角形であるベガ・アルタイル・デネブは空高くにいます", Nil),
    ("20h41m", "はくちょう座デネブが南中。天頂付近にいます。夏の大三角形の1つです。デネブの西はこと座ベガです", Nil),
    // TODO こぎつね、いるか、こうま

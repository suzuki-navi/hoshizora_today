...

{
  ...
  val stars = IndexedSeq(
    ...
    ("05h15m", "オリオン座リゲルが南中。冬のダイヤモンドを構成する6個の星の1つです", List("オリオン座")),
    ("05h17m", "ぎょしゃ座カペラが南中。天頂付近にいます。冬のダイヤモンドを構成する6個の星の1つです", Nil),
    ("05h55m", "オリオン座ベテルギウスが南中。冬のダイヤモンドの中心の星です", List("オリオン座")),
    // TODO うさぎ
    ...
    ("12h16m", "からす座が南中。からす座は、おとめ座スピカの南西(右下)にある目立たない星座です", Nil),
    ...
    ("15h44m", "へび座α(3等星)が南中。へび座でもっとも明るい星で、へびの頭にあります", Nil),
